...
import { Search, Filter, X, Download, HardHat, Loader2 } from "lucide-react";
...
export default function InstallationPage() {
  ...
  const getFiltered = (tabValue: string) => {
    ...
    switch (tabValue) {
      case "ready":
        list = orders.filter((o) => o.dispatchedWindows > 0 && o.installedWindows === 0);
        break;
      case "partial":
        list = orders.filter((o) => o.installedWindows > 0 && o.installedWindows < o.atw);
        break;
      case "full":
        list = orders.filter((o) => o.installedWindows > 0 && o.installedWindows >= o.atw && o.atw > 0);
        break;
    }
  ...
  const readyForInstallCount = orders.filter(o => o.dispatchedWindows > 0 && o.installedWindows === 0).length;
  ...
  const handleExport = () => {
    ...
    const data = filtered.map(o => ({
      ...
      "Dispatched": o.dispatchedWindows,
      "Installed": o.installedWindows,
      "Pending": o.dispatchedWindows - o.installedWindows
    }));
    exportDataToExcel(data, headers, `installation_export_${tab}.xlsx`);
  };

  return (
    <PageWrapper title="Installation Management">
      <PageHeader 
        title="Site Ops Hub" 
        subtitle={`${readyForInstallCount} PROJECTS AWAITING SITE DEPLOYMENT`}
        icon={HardHat}
      >
        <Button variant="outline" size="sm" className="h-10 text-[10px] font-black uppercase tracking-[0.1em] rounded-xl border-slate-200" onClick={handleExport}>
          <Download className="mr-2 h-4 w-4" /> Export Protocol
        </Button>
      </PageHeader>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
         <Tabs value={tab} onValueChange={setTab} className="w-full lg:w-auto">
            <TabsList className="bg-slate-100/50 p-1 h-11 rounded-2xl border border-slate-200/50">
              ...
              <TabsTrigger value="partial" className="rounded-xl px-4 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm">In Progress</TabsTrigger>
              <TabsTrigger value="full" className="rounded-xl px-4 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm">Certified</TabsTrigger>
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 min-w-[300px]">
              ...
              <Input 
                 placeholder="SCAN SITE PROTOCOLS..." 
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-slate-100 hover:bg-transparent">
                ...
                <TableHead className="py-5 font-black text-[10px] uppercase tracking-widest text-slate-400">Engagement Metrics</TableHead>
                <TableHead className="py-5 font-black text-[10px] uppercase tracking-widest text-slate-400">Site Status</TableHead>
                <TableHead className="py-5 font-black text-[10px] uppercase tracking-widest text-slate-400 text-right px-6">Residual</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-32">
                     <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">Synchronizing Site Stream...</p>
                     </div>
                  </TableCell>
                </TableRow>
              ) : filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-32">
                     <div className="flex flex-col items-center gap-4 opacity-50">
                        <HardHat className="h-12 w-12 text-slate-200" />
                        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No site deployments active</p>
                     </div>
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <TableCell>
                       <div className="flex flex-col gap-1.5 min-w-[180px]">
                          <div className="flex items-baseline gap-2">
                             <span className="text-[14px] font-black text-slate-900">{o.installedWindows}</span>
                             <span className="text-[9px] font-black text-slate-300 uppercase">/ {o.dispatchedWindows} AT SITE</span>
                          </div>
                          <div className="flex items-center gap-2">
                             <div className="h-1.5 flex-1 bg-slate-100 rounded-full overflow-hidden">
                                <div 
                                  className="h-full bg-emerald-500 transition-all duration-1000" 
                                  style={{ width: `${(o.installedWindows / (o.dispatchedWindows || 1)) * 100}%` }} 
                                />
                             </div>
                             <span className="text-[9px] font-black text-slate-400 whitespace-nowrap">{Math.round((o.installedWindows / (o.atw || 1)) * 100)}% TOTAL</span>
                          </div>
                       </div>
                    </TableCell>
                    <TableCell>
                       <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                             <StatusDot status={o.installation_status === "Completed" ? "green" : o.installation_status === "WIP" ? "amber" : "grey"} />
                             <span className={cn("font-black", o.installation_status === "Completed" ? "text-emerald-600" : o.installation_status === "WIP" ? "text-amber-600" : "text-slate-400")}>
                               {o.installation_status || "PENDING"}
                             </span>
                          </div>
                          <span className="text-[9px] text-slate-400 font-medium">{o.product_type}</span>
                       </div>
                    </TableCell>
                    <TableCell className="px-6 text-right">
                       <div className="flex flex-col items-end">
                          <span className="text-[14px] font-black text-red-500">{o.dispatchedWindows - o.installedWindows}</span>
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">UNINSTALLED</span>
                       </div>
                    </TableCell>
                  </TableRow>
                ))

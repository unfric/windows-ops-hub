...
import { Search, Filter, X, Download, Truck, Loader2 } from "lucide-react";
...
export default function DispatchPage() {
  ...
  const getFiltered = (tabValue: string) => {
    ...
    switch (tabValue) {
      case "ready":
        list = orders.filter((o) => o.productionPacked > 0 && o.dispatchedWindows === 0);
        break;
      case "partial":
        list = orders.filter((o) => o.dispatchedWindows > 0 && o.dispatchedWindows < o.designReleased);
        break;
      case "full":
        list = orders.filter((o) => o.dispatchedWindows > 0 && o.dispatchedWindows >= o.designReleased && o.designReleased > 0);
        break;
    }
  ...
  const readyForDispatchCount = orders.filter(o => o.productionPacked > 0 && o.dispatchedWindows === 0).length;
  ...
  const handleExport = () => {
    ...
    const data = filtered.map(o => ({
      ...
      "Ready": o.productionPacked,
      "Dispatched": o.dispatchedWindows,
      "Balance": o.productionPacked - o.dispatchedWindows
    }));
    exportDataToExcel(data, headers, `dispatch_export_${tab}.xlsx`);
  };

  return (
    <PageWrapper title="Logistics & Dispatch">
      <PageHeader 
        title="Nexus Post" 
        subtitle={`${readyForDispatchCount} SHIPMENTS READY FOR DEPLOYMENT`}
        icon={Truck}
      >
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
         <Tabs value={tab} onValueChange={setTab} className="w-full lg:w-auto">
            <TabsList className="bg-slate-100/50 p-1 h-11 rounded-2xl border border-slate-200/50">
              <TabsTrigger value="ready" className="rounded-xl px-4 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm">Ready</TabsTrigger>
              <TabsTrigger value="partial" className="rounded-xl px-4 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm">Partial</TabsTrigger>
              <TabsTrigger value="full" className="rounded-xl px-4 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:shadow-sm">Deployed</TabsTrigger>
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 min-w-[300px]">
              ...
              <Input 
                 placeholder="SCAN LOGISTICS DATA..." 
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-slate-100 hover:bg-transparent">
                ...
                <TableHead className="py-5 font-black text-[10px] uppercase tracking-widest text-slate-400">Logistics Metrics</TableHead>
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-32">
                     <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">Synchronizing Logistics Stream...</p>
                     </div>
                  </TableCell>
                </TableRow>
              ) : filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-32">
                     <div className="flex flex-col items-center gap-4 opacity-50">
                        ...
                        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">No logistics audit required</p>
                     </div>
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={o.id} className="group/row hover:bg-slate-50/80 transition-all border-slate-100 h-20">
                    ...
                    <TableCell>
                       <div className="flex flex-col gap-1.5 min-w-[180px]">
                          <div className="flex items-baseline gap-2">
                             <span className="text-[14px] font-black text-slate-900">{o.productionPacked}</span>
                             <span className="text-[9px] font-black text-slate-300 uppercase">/ {o.designReleased} READY</span>
                          </div>
                          <div className="flex items-center gap-2">
                             <div className="h-1.5 flex-1 bg-slate-100 rounded-full overflow-hidden">
                                <div 
                                  className="h-full bg-emerald-500 transition-all duration-1000" 
                                  style={{ width: `${(o.dispatchedWindows / (o.productionPacked || 1)) * 100}%` }} 
                                />
                             </div>
                             <span className="text-[9px] font-black text-slate-400 whitespace-nowrap">{o.dispatchedWindows} DISPATCHED</span>
                          </div>
                       </div>
                    </TableCell>
                    <TableCell>
                       <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                             ...
                             <span className={cn("font-black", o.approval_for_dispatch === "Approved" ? "text-emerald-600" : o.approval_for_dispatch === "Hold" ? "text-amber-600" : "text-slate-400")}>
                               DISP: {o.approval_for_dispatch}
                             </span>
                          </div>
                    ...
                    <TableCell className="px-6 text-right">
                       <div className="flex flex-col items-end">
                          <span className="text-[11px] font-black text-slate-900">₹{Number(o.order_value).toLocaleString()}</span>
